...
const Header: React.FC = () => {
  ...
  const [themeType, setThemeType] = useRecoilState(themeAtom)
  ...
  return (
    <Wrapper>
      <Navbar>
        <LogoLink>
          <NavLink to='/'>
            <Logo width='143px' height='35px' color={themeConstants[themeType].primary} />
          </NavLink>
        </LogoLink>
        ...
        <SearchButton theme={theme} onClick={() => navigate('/search')}>
          <SearchOutlined />
        </SearchButton>
        ...
        {localStorage.getItem('auth') ? (
          <>
            <NavLink to='/questions'>
              <StandardLink theme={theme}>Questions</StandardLink>
            </NavLink>
            <Button
              onClick={() => {
